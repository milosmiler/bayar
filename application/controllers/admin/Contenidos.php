...
class Contenidos extends CI_Controller {
    ...
    public function listado_contenidos()
    {
        ...
        $this->load->view("admin/listado_contenidos");
        $this->load->view("admin/layouts/footer");
    }


    public function eliminar_contenidos($slug)
    {
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            $this->load->model("Contenidos_Model", "contenidos");
            $datos = $this->contenidos->getAllPropertiesOnly($slug);

            if ($datos) {
                @unlink('./uploads/post/contenidos/'.$datos->imagen1);
                @unlink('./uploads/post/contenidos/'.$datos->imagen2);
                @unlink('./uploads/post/contenidos/'.$datos->imagen3);
                @unlink('./uploads/post/contenidos/'.$datos->imagend1);
                @unlink('./uploads/post/contenidos/'.$datos->imagend2);
                @unlink('./uploads/post/contenidos/'.$datos->imagend3);
                @unlink('./uploads/post/contenidos/'.$datos->imagend4);
                @unlink('./uploads/post/contenidos/'.$datos->imagend5);
                @unlink('./uploads/post/contenidos/'.$datos->imagen4);

                if ($this->contenidos->eliminarEvent($slug)) {
                    $this->session->set_flashdata('success', 'Se elimino con exito la Activacion');
                    redirect(base_url("admin/proyectos/contenidos/listado"));
                }
                else {
    ...
    public function crear_contenidos()
    {
        $this->load->library('session');
        $this->load->helper('form');
    ...
    public function editar_contenidos($slug)
    {
        $slug = urldecode($slug);
        $this->load->library('session');
        $this->load->helper('form');
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            $this->load->model("Contenidos_Model", "contenidos");
            $datos = $this->contenidos->validateDataPost($slug, "contenidos");
    ...
    public function post_editar($slug)
    {
       ...
       $this->load->model("Contenidos_Model", "contenidos");
        ...
        $config['upload_path']   = './uploads/post/contenidos';
        ...
        //actualizar datos
        if (!$slres = $this->contenidos->updateAllProperties($data, $dataup, $slug)) {
            $this->deshacerCambios($dataup);
        ...
        if ($slres != $slug) {
            ...
            redirect(base_url("admin/proyectos/contenidos/editar/".$slres));
            return false;
        }
    ...
    public function post_create()
    {
        ...
        //actualizar datos
        if (!$this->contenidos->insertAllProperties($data, $dataup)) {
            $this->deshacerCambios($dataup);
            $data["error_update"] = "No se pudo actualizar la base de datos, intentelo más tarde";
