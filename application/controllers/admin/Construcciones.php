...
class Construcciones extends CI_Controller {
    ...
    public function listado_construcciones()
    {
        ...
        $data["menu"] = "construcciones";
        $data["datos"] = $this->construcciones->getAllProperties();

        $this->load->view("admin/layouts/header", $data);
        $this->load->view("admin/listado_construcciones");
        $this->load->view("admin/layouts/footer");
    }


    public function eliminar_construcciones($slug)
    {
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            $this->load->model("Construcciones_Model", "construcciones");
            $datos = $this->construcciones->getAllPropertiesOnly($slug);

            if ($datos) {
                @unlink('./uploads/post/construcciones/'.$datos->imagen1);
                @unlink('./uploads/post/construcciones/'.$datos->imagen2);
                @unlink('./uploads/post/construcciones/'.$datos->imagen3);
                @unlink('./uploads/post/construcciones/'.$datos->imagend1);
                @unlink('./uploads/post/construcciones/'.$datos->imagend2);
                @unlink('./uploads/post/construcciones/'.$datos->imagend3);
                @unlink('./uploads/post/construcciones/'.$datos->imagend4);
                @unlink('./uploads/post/construcciones/'.$datos->imagend5);
                @unlink('./uploads/post/construcciones/'.$datos->imagen4);

                if ($this->construcciones->eliminarEvent($slug)) {
                    $this->session->set_flashdata('success', 'Se elimino con exito el Evento');
                    redirect(base_url("admin/proyectos/construcciones/listado"));
                }
                else {
                    ...
                    //vista
                    $data["nombre_admin"] = $this->session->userdata('nombre');
                    $data["datos"] = $this->construcciones->getAllPropertiesOnly($slug);
                    $data["menu"] = "construcciones";
    ...
    public function crear_construcciones()
    {
        $this->load->library('session');
        $this->load->helper('form');
        ...
        $this->load->view("admin/layouts/header", $data);
        $this->load->view("admin/crear_construcciones");
        $this->load->view("admin/layouts/footer");
    }


    public function editar_construcciones($slug)
    {
        $slug = urldecode($slug);
        $this->load->library('session');
        $this->load->helper('form');
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            $this->load->model("Construcciones_Model", "construcciones");
            $datos = $this->construcciones->validateDataPost($slug, "construccion");
    ...
    public function post_editar($slug)
    {
       ...
       $this->load->model("Construcciones_Model", "construcciones");
        ...
        $config['upload_path']   = './uploads/post/construcciones';
        ...
        //actualizar datos
        if (!$slres = $this->construcciones->updateAllProperties($data, $dataup, $slug)) {
            $this->deshacerCambios($dataup);
        ...
        if ($slres != $slug) {
            ...
            redirect(base_url("admin/proyectos/construcciones/editar/".$slres));
            return false;
        }
    ...
    public function post_create()
    {
        ...
        //actualizar datos
        if (!$this->construcciones->insertAllProperties($data, $dataup)) {
            $this->deshacerCambios($dataup);
            $data["error_update"] = "No se pudo actualizar la base de datos, intentelo más tarde";
    ...
    public function deshacerCambios($imagenes)
    {
        ...
        if ($imgsd1 != null) {
            @unlink('./uploads/post/construcciones/'.$imgsd1);
        }


        @$imgsd2 = $imagenes["uploadDataSD2"]["file_name"];
        if ($imgsd2 != null) {
            @unlink('./uploads/post/construcciones/'.$imgsd2);
        }

        @$imgsd3 = $imagenes["uploadDataSD3"]["file_name"];
        if ($imgsd3 != null) {
            @unlink('./uploads/post/construcciones/'.$imgsd3);
        }

        @$imgsd4 = $imagenes["uploadDataSD4"]["file_name"];
        if ($imgsd4 != null) {
            @unlink('./uploads/post/construcciones/'.$imgsd4);
        }

        @$imgsd5 = $imagenes["uploadDataSD5"]["file_name"];
        if ($imgsd5 != null) {
            @unlink('./uploads/post/construcciones/'.$imgsd5);
        }
