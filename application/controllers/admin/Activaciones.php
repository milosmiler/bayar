...
class Activaciones extends CI_Controller {
    ...
    public function listado_activaciones()
    {
        ...
        $data["datos"] = $this->activaciones->getAllProperties();

        $this->load->view("admin/layouts/header", $data);
        $this->load->view("admin/listado_activaciones");
    ...
    public function eliminar_activaciones($slug)
    {
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            if ($datos) {
                ...
                @unlink('./uploads/post/activaciones/'.$datos->imagend1);
                @unlink('./uploads/post/activaciones/'.$datos->imagend2);
                @unlink('./uploads/post/activaciones/'.$datos->imagend3);
                @unlink('./uploads/post/activaciones/'.$datos->imagend4);
                @unlink('./uploads/post/activaciones/'.$datos->imagend5);
    ...
    public function editar_activaciones($slug)
    {
        ...
        if ($this->input->post("submit") && $_POST) {
            ...
            $this->load->model("Activaciones_Model", "activaciones");
            $datos = $this->activaciones->validateDataPost($slug, "activacion");
    ...
    public function post_editar($slug)
    {
        ...
        $config['upload_path']   = './uploads/post/activaciones';
        ...
        //actualizar datos
        if (!$slres = $this->activaciones->updateAllProperties($data, $dataup, $slug)) {
            $this->deshacerCambios($dataup);
        ...
        if ($slres != $slug) {
            ...
            redirect(base_url("admin/proyectos/activaciones/editar/".$slres));
            return false;
        }
    ...
    public function post_create()
    {
        ...
        //actualizar datos
        if (!$this->activaciones->insertAllProperties($data, $dataup)) {
            $this->deshacerCambios($dataup);
            $data["error_update"] = "No se pudo actualizar la base de datos, intentelo más tarde";
    ...
    public function deshacerCambios($imagenes)
    {
        ...
        if ($imgs1 != null) {
            @unlink('./uploads/post/activaciones/'.$imgs1);
        }

        @$imgs2 = $imagenes["uploadDataS2"]["file_name"];
        if ($imgs2 != null) {
            @unlink('./uploads/post/activaciones/'.$imgs2);
        }

        @$imgs3 = $imagenes["uploadDataS3"]["file_name"];
        if ($imgs3 != null) {
            @unlink('./uploads/post/activaciones/'.$imgs3);
        }
        ...
        if ($imgs4 != null) {
            @unlink('./uploads/post/activaciones/'.$imgs4);
        }
